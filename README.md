...
Bottom line: the pilots above are single-run-per-condition results, not a validated benchmark. The one consistent finding from them was that the skills only changed an outcome when a weaker model (Haiku-tier) faced a task with a real trap in it — on easy tasks or with a stronger baseline model, the skills added token/cost overhead without measurable benefit.

**Follow-up: full Haiku tier at n=5.** To test whether the pilot's single-run signal held up, `agent-artifacts/run_full_haiku_eval.py` reran the same baseline-vs-treatment split at n=5 reps across 12 tasks: the 6 original adversarial fixtures, 4 new harder fixtures (one per always-on/gnarly skill — a rename consumed through a dynamic string key, a two-hop root-cause bug with a tempting-but-wrong intermediate fix, a stale-cache failure that misattributes blame, and a test that contradicts its own spec), and 2 easy control tasks. 120 real Claude Code runs on `haiku`, cost $5.51. Full results, per-task breakdown, and Fisher's-exact-test tables are in `agent-artifacts/EVAL_RESULTS_HAIKU_FULL.md`.

- Pooled across all 10 trap tasks (n=50/condition): baseline caught 72.0%, treatment caught 78.0% — a +6.0pp difference, not significant (Fisher p=0.65), and below the pre-committed 15pp bar for "worthwhile at this tier."
- The token/cost overhead replicated cleanly: treatment cost +36%, tokens +49%, wall-clock +33%, with zero regression on the two easy control tasks (5/5 both conditions).
- The benefit wasn't spread evenly across skills — it concentrated in the `multi-file-atomic-edits` fixtures (3/5→5/5 and 1/5→5/5), while two other tasks went the other way (5/5→3/5 and 2/5→0/5).
- Reading: **inconclusive, trending toward "not worthwhile" at the Haiku tier** — the pilot's 2/6 differential mostly did not replicate at n=5. This is evidence the pilot's original signal was largely noise, not evidence the skills don't work; the Sonnet/Opus tiers in `agent-artifacts/TESTING_PLAN.md` haven't been run yet, and the 4 new harder fixtures (validated to produce genuine baseline failures, e.g. 1/5 on `deep_consumer_rename`) are built for reuse there.
